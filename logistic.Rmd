...
test_features<-test_features%>%dplyr::select(starts_with('c') | starts_with('g'))
...
test_features<-predict(pca_cg, test_features)
print(glue("Completed PCA!"))
```

$//$ Train models: $//$
```{r}
cl<-makeCluster(10)     # modify this to number of parallel processes required
...
models<-foreach(i=1:length(predictors)  ,.packages=c("glue","dplyr","speedglm")) %dopar% {
  ...
  speedglm(train_y_predictor~ . ,data = data.frame(train_x_pca), family=binomial(), maxit = 250)
}
stopCluster(cl)
end_time<-Sys.time()
...
preds<-foreach(i=1:length(predictors)  ,.packages=c("glue","dplyr","speedglm")) %do% {
  predict(models[[i]],newdata = test_x_pca,type="response")
}
```
